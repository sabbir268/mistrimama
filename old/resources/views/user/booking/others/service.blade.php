...
<section class="box-typical steps-icon-block">
    ...
    <section class="box-typical-section bg-white rounde ">
            ...
            <form action="{{route('servicec_store')}}" class="step1Form"
                method="post" data-category-id="{{$data->id}}" data-base-price="{{$data->price}}"
                data-additional-price="{{$data->additional_price}}">
                {{csrf_field()}}
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="row pt-0 pm-0">
                                        <h5 class="mb-0 border-bottom">{{$data->name}}</h5>
                                    </div>
                                    <div class="row mb-0">
                                        <p class="m-0">
                                            <span>Price: ৳
                        ...
                        <div class="row">
                            ...
                            <div class="input-group mb-3">
                                    <div class="input-group-prepend">
                                         <button type="button" class="btn btn-sm btn-danger mr-0" id="decrease-{{$data->id}}" onclick="decreaseQuantity(this, {{$data->id}})" data-base-price="{{$data->price}}" data-additional-price="{{$data->additional_price}}"><i class="fa fa-minus"></i></button>
                                    </div>
                                    <input type="text" value="1" class="btn btn-sm bg-white rounded-0 text-center mr-0 ml-0" name="qty" placeholder="Qty" id="increment{{$data->id}}">
                                    <div class="input-group-append">
                                            <button type="button" class="btn btn-success ml-0" id="increase-{{$data->id}}" onclick="increaseQuantity(this, {{$data->id}})" data-base-price="{{$data->price}}" data-additional-price="{{$data->additional_price}}"><i class="fa fa-plus"></i></button>
                                    </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <button type="submit" class="btn btn-sm  pull-right" id="{{$data->id}}" data-cat-id="{{$data->id}}">ADD</button>
                        </div>
                        </div>
                    </header>
                </section>
    ...
    <button type="button" class="btn btn-rounded btn-grey float-left"> <a href="{{route('book-self')}}">← Back</a></button>
    <button type="submit" class="btn btn-rounded float-right"> <a href="{{route('date-time')}}"> Next →</a></button>
